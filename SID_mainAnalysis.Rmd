...
- 'self' contains ratings of artworks. The originally imported data (in SID_cleaning.Rms; [SID = shared individual differences]) contains data on people's prediction of lay people's and art experts' ratings of the images (We don't use this data in this script).
...
self <-  data.frame(read.csv("data/SID_selfRatings.csv"), stringsAsFactors = F)[,2:275]
...
K <- 35 # optimal taste cohort size
```
...
write.csv(rmse.neigh.mean, "data/rmse.neigh.mean.csv")
write.csv(rmse.other.mean, "data/rmse.other.mean.csv")
```

If you prefer not to run the code chunk above, here, we import the output of the previous code. Basically, we are just reimporting the outcome of taste_cohort, so we don't have to run that chunk every time we want to perform some analyses. 
```{r taste_cohort_clean_IF_import}
rmse.neigh.mean <- read.csv("data/rmse.neigh.mean.csv")  %>% data.frame() %>% select(2:ncol(rmse.neigh.mean))
rmse.other.mean <- read.csv("data/rmse.other.mean.csv") %>% data.frame() %>% select(2:ncol(rmse.other.mean))
```
...
d <- rmse.mMeans %>% cohens_d(rmse ~ neigh, paired=TRUE)
...
ggplot(combined_rmse, aes(m_size, rmse, color=neigh)) + geom_point(show.legend = F, size=0.5) +
  ...
  scale_x_continuous(breaks = seq(0, 274, by = 20)) + 
  theme(axis.title = element_text(size = 20), axis.text.x=element_text(size=10)) +
  annotate("text", x=c(250,250), y=c(1.60,1.52), label=c("Random cohort", "Taste cohort"),
           color=c("#00BFC4","#FB766D"), fontface="bold", size=5)
```

(3.2) Optimum taste cohort size
```{r optimum_taste_cohort}
### optimum cohort size of mean of all iterations - it's 31
...
### optimum cohort size at each iteration
...
write.csv(opt_cohort, "data/opt_cohort_size.csv")
...
(3.3) Comparing taste cohort models with sample mean models
```{r taste_cohort_sample_mean}
# extract the RMSE when the cohort size is optimized; i.e., 31
rmse.31 <- rmse.neigh.mean %>% data.frame() %>% select(X31) 
...
t.test(rmse.sample$.,rmse.31$X31, paired=T)

# calculating the effect size:
d <- cbind(rmse.sample$.,rmse.31$X31) %>% data.frame() %>% 
...
N.LAST <- SS - 3
ss_set <- seq(from=10, to=SS, by=1) # sample size
...
#write.csv(min.point.mean, "/Users/field-admin/Library/CloudStorage/OneDrive-CornellUniversity/Desktop/min_point_mean.csv")

```

```{r taste_cohort_sampleSize_plots}
min.point.mean <- read.csv("data/min_point_mean.csv")[,2] %>% data.frame() %>% rename("ocs"=1)
min.point.mean$ss <- seq(from=10, to=nrow(min.point.mean)+9, by=1)
min.point.mean$ss_log <- log(min.point.mean$ss)
fit_ss <- lm(ocs ~ ss_log,  data = min.point.mean)
min.point.mean$predictions <- fit_ss[["fitted.values"]]

ggplot(min.point.mean, aes(ss, ocs)) + 
  geom_point(show.legend = T, alpha=0.5) + 
  geom_line(aes(ss, predictions), size=1, color="blue") + 
...
for (r in 1:100) {
  ...
  colnames(distance.matrix) <- rownames(distance.matrix) <- colnames(self)
    ...
    for (i in 1:SS) { #loop through each subject
      ...
      RSS.neigh = sum((main.subject.test$DV - preds.neigh)^2)
      TSS = sum((main.subject.test$DV - mean(main.subject.test$DV))^2)
      
      R2.neigh = 1 - (RSS.neigh/TSS)
      r2.llvp.cohort[i, r] <- R2.neigh
      rmse.llvp.cohort[i, r] <- sqrt(mean((main.subject.test$DV - preds.neigh)^2))
    }
}

# RMSE
rmse.llvp.cohort[rmse.llvp.cohort == "-Inf" ] <- 0
...
llvp.cohort.rmse.itM <- rowMeans(rmse.llvp.cohort,na.rm=T) %>% data.frame() 

# R2
r2.llvp.cohort[r2.llvp.cohort == "-Inf" ] <- 0
llvp.cohort.r2M <- rowMeans(r2.llvp.cohort,na.rm=T) %>% data.frame() %>% colMeans() %>% print()
llvp.cohort.r2SD <- rowMeans(r2.llvp.cohort,na.rm=T) %>% sd() %>% print()
llvp.cohort.r2.itM <- rowMeans(r2.llvp.cohort,na.rm=T) %>% data.frame() 
```
...
t.test(combined_models$taste_cohort, combined_models$sample_mean, paired = TRUE)
t.test(combined_models$visual_features, combined_models$sample_mean, paired = TRUE)

d <- all.RMSE %>% 
  filter(model == c("Taste cohort\n models", "Visual features\n model")) %>% 
  cohens_d(rmse ~ model, paired=TRUE)

# visualization
ggplot(all.RMSE, aes(x=model, y=rmse)) + 
  geom_boxplot() + 
...
b = emo %>% cohens_d(difference ~ cohort, paired=TRUE)
...
b = agree %>% cohens_d(difference ~ cohort, paired=TRUE)
...
b = extra %>% cohens_d(difference ~ cohort, paired=TRUE)
...
b = art %>% cohens_d(difference ~ cohort, paired=TRUE)
...
b = df_subtracted %>% cohens_d(mean_deviation ~ gender, paired=TRUE)
ggplot(df_subtracted, aes(x=gender, y=mean_deviation)) + geom_boxplot() + ylab("Mean rating deviation (atypicality)")
```

```{r llvp_individual_models_NOTUSED}
set.seed(12345)

R2.ind.model <- data.frame()
...
for (r in 1:10) { 
  ...
  for (s in 10:ncol(llvp_self_train)) { 
    ...
    RSS = sum((data.test.IDs[,s-9] - model_predictions)^2)
    TSS = sum((data.test.IDs[,s-9] - mean(data.test.IDs[,s-9]))^2)
    R2.ind.model[s-9,r] <- 1 - (RSS/TSS)
    
    rmse.ind.model[s-9,r] <- sqrt(mean((data.test.IDs[,s-9] - model_predictions)^2))
  }
}

hist((R2.ind.model) %>% unlist(), breaks = 100)
...
# R2
R2.ind.model[ R2.ind.model == "-Inf" ] <- 0
llvp.ind.r2M <- rowMeans(R2.ind.model,na.rm=T) %>% data.frame() %>% colMeans() %>% print()
llvp.ind.r2SD <- rowMeans(R2.ind.model,na.rm=T) %>% sd() %>% print()
llvp.ind.r2.it <- rowMeans(R2.ind.model,na.rm=T) %>% data.frame()
```

```{r llvp_mean_models_NOTUSED}
set.seed(1234)

R2.mean.model = data.frame()
...
for (r in 1:10) { 
  ...
  for (s in 1:ncol(self)) { 
    RSS = sum((data.test.IDs[,s] - model_predictions)^2)
    TSS = sum((data.test.IDs[,s] - mean(data.test.IDs[,s]))^2)
    R2.mean.model[s,r] =  1 - (RSS/TSS)
    rmse.mean.model[s, r] <- sqrt(mean((data.test.IDs[,s] - model_predictions)^2))
  }
}
hist(unlist(R2.mean.model %>% filter(V1 > -2)), breaks = 100)
...
# R2
R2.mean.model[R2.mean.model == "-Inf" ] <- 0
llvp.mean.r2M <- rowMeans(R2.mean.model,na.rm=T) %>% data.frame() %>% colMeans() %>% print()
llvp.mean.r2SD <- rowMeans(R2.mean.model,na.rm=T) %>% sd() %>% print()
llvp.mean.r2.it <- rowMeans(R2.mean.model,na.rm=T) %>% data.frame() 
```